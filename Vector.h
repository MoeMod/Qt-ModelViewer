...
public:
	...
	inline Vector operator-=(const Vector &v) { return Vector(x -= v.x, y -= v.y, z -= v.z); }
	inline Vector operator*=(const Vector &v) { return Vector(x *= v.x, y *= v.y, z *= v.z); }
	inline Vector operator/=(const Vector &v) { if (!v.x || !v.y || !v.z) return *this;  Vector(x /= v.x, y /= v.y, z /= v.z); }
	inline Vector operator*=(float fl) { return Vector(x *= fl, y *= fl, z *= fl); }
	inline Vector operator/=(float fl) { if (!fl) return *this; return Vector(x /= fl, y /= fl, z /= fl); }
